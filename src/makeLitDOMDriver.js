...
import { map, startWith, shareReplay, takeUntil, switchMap, } from 'rxjs/operators';
import { render } from 'lit-html';
import { LitDOMSource } from './LitDOMSource';
import { IsolateModule } from './IsolateModule';
import { EventDelegator } from './EventDelegator';
import { checkValidContainer, getValidNode, defaultReportRenderError, } from './utils';
function domDriverInputGuard(view$) {
    if (!view$) {
        throw new Error(`The lit-html DOM driver function expects as input a stream of ` +
            `lit-html templates`);
    }
}
...
function addRootScope(template, namespace) {
...
function processIsolatedElements(mutations, isolateModule, template) {
    // Collect all isolated template metadata
    const isolatedTemplates = new Map();
...
function collectIsolatedTemplates(template, isolated) {
    if (!template)
        return;
    ...
    if (template &&
        ...
        const signature = template._isolate.map((s) => s.scope || s).join('-');
...
function registerElementIfIsolated(element, isolatedTemplates, isolateModule) {
...
export function makeLitDOMDriver(container, options = {}) {
    ...
    function LitDOMDriver(template$, name = 'LitDOM') {
        domDriverInputGuard(template$);
        const sanitation$ = new Subject();
        const firstRoot$ = domReady$.pipe(map(() => {
            const firstRoot = getValidNode(container) || document.body;
            return firstRoot;
        }));
        // Convert the SinkProxy/xstream to RxJS Observable
        const templateObservable$ = from(template$);
        const rememberedTemplate$ = templateObservable$.pipe(shareReplay(1));
        mutationConfirmed$.subscribe();
        const elementAfterRender$ = firstRoot$.pipe(map(firstRoot => {
            return merge(rememberedTemplate$.pipe(takeUntil(sanitation$)), sanitation$).pipe(map(template => {
                if (template === null) {
                    return firstRoot;
                }
                try {
                    ...
                    const reportError = options.reportRenderError || defaultReportRenderError;
                    reportError(error);
                    return firstRoot;
                }
            }), startWith(firstRoot), dropCompletion);
        }), switchMap(x => x));
        const rootElement$ = merge(domReady$, mutationConfirmed$).pipe(takeUntil(sanitation$), switchMap(() => elementAfterRender$), shareReplay(1));
        ...
        return new LitDOMSource(rootElement$, sanitation$, [], isolateModule, delegator, name);
    }
    return LitDOMDriver;
}
//# sourceMappingURL=makeLitDOMDriver.js.map